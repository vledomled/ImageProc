...
# Загрузка коэффициентов Бокастена
...
# Загрузка данных
file_path = 'gauss.xlsx'  # Укажите путь к вашему файлу
...
for i, column in enumerate(intensity_columns.columns):
    ...
    # Фиттинг Гаусса с центром в нулевом радиусе
    popt, _ = curve_fit(gaussian, radii, intensities, p0=[np.max(intensities), 0, 0.5])
    smoothed_intensities = gaussian(fine_radii, *popt)

    # Построение графика аппроксимации Гаусса
    ...
    r_max = radii.max()  # Максимальный радиус для текущей линии
    ...
    # Построение графика абелизации
    plt.figure(figsize=(10, 6))
    plt.plot(radii, intensities, 'o', label='Original Data', color='blue')
    plt.plot(radii, abelized_intensities, 's', label='Abelized Data (Original Radii)', color='red')
    plt.xlabel('Radius (mm)')
    plt.ylabel('Intensity')
    plt.title(f'Abel Transformation for Line {i+1}')
    plt.legend()
    plt.grid()
    plt.show()

# Сохранение результатов абелизации и аппроксимации Гаусса
